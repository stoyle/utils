...
const pathnameBuilder = (...args) => {
    ...
    const parts = args
        .filter(
            arg => (Array.isArray(arg) && arg.length !== 0) || (isString(arg) && arg.length !== 0),
        )
        .map(arg => {
...
const uriBuilder = (input = '', base = '', extra = '') => {
    ...
    const basePath = uriObj.pathname.split('/').filter(item => item && !item.includes('.json'));
...
const duplicateOnLocalsPodium = (response = {}, fromProperty, toProperty) =>
    setAtLocalsPodium(response, toProperty, getFromLocalsPodium(response, fromProperty));
